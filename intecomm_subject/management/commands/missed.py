...
from intecomm_prn.models import EndOfStudy, OffScheduleComm, OffScheduleInte
from intecomm_subject.models import SubjectVisit, SubjectVisitMissed
...
def get_visit_codes(subject_identifier: str) -> list[str]:
    """Return visit codes for subject_identifier."""
    codes = []
...
class Result:
    ...
    def formatted(self) -> str:
        ...
        return (
            ...
            f"{self.attended_after_missed},"
            f"{self.offschedule},{self.offstudy},{formatted_date(self.offstudy_date)},"
            f"{';'.join(self.visit_codes)}"
        )


def get_offschedule_date(subject_identifier: str) -> date:
    dt = None
    try:
        offschedule = OffScheduleInte.objects.get(
            subject_identifier=subject_identifier
        ).offschedule_datetime
    except ObjectDoesNotExist:
        try:
            offschedule = OffScheduleComm.objects.get(
                subject_identifier=subject_identifier
            ).offschedule_datetime
        except ObjectDoesNotExist:
            offschedule = None
...
def runner(result=None):
    ...
    for subject_identifier, vcodes in result.items():
        ...
        offschedule = get_offschedule_date(subject_identifier)
