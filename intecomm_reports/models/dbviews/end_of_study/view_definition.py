...
def get_view_definition() -> dict:
    ...
    from (
        ...
        offstudyreasons.name as offstudy_reason, appt.visit_code, appt.visit_code_sequence, appt.site_id
		from (
			...
			max(appt.visit_code_sequence) as visit_code_sequence
		...
		left join edc_appointment_appointment as appt on (b1.subject_identifier=appt.subject_identifier and b1.timepoint=appt.timepoint and appt.visit_code_sequence=b1.visit_code_sequence)
        left join intecomm_prn_endofstudy as eos on eos.subject_identifier=b1.subject_identifier
        left join intecomm_lists_offstudyreasons as offstudyreasons on eos.offstudy_reason_id=offstudyreasons.id
        left join edc_visit_schedule_subjectschedulehistory as sched on b1.subject_identifier=sched.subject_identifier
        group by b1.subject_identifier, sched.schedule_status, offstudyreasons.name, appt.visit_code, appt.visit_code_sequence, appt.site_id
