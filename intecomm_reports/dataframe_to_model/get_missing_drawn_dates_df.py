...
def get_missing_drawn_dates_df(model: str | None = None):
    ...
    df_review = df_review.rename(
        columns={"baseline_datetime": "baseline_date", "visit_datetime": "visit_date"}
    )
    df_review = df_review[(df_review["has_vl"] == YES) & (df_review["drawn_date"].isna())]
    df_review = df_review[keep_cols]
    ...
    df_initial = df_initial[(df_initial["has_vl"] == YES) & (df_initial["drawn_date"].isna())]
    df_initial = df_initial[keep_cols]
    ...
    df_missing_drawn_date = pd.concat([df_review, df_initial])
